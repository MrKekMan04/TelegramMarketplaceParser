...
import ru.overcode.scrapper.service.marketplace.AliexpressService;
...
public class AliexpressProcessor implements MarketplaceProcessor {
    ...
    @Value("${feign.aliexpress.link.batch-size}")
    private final int linksBatchSize;
    @Value("${feign.aliexpress.rule.batch-size}")
    private final int rulesBatchSize;

    @Override
    public MarketName getMarketName() {
        return MarketName.ALIEXPRESS;
    }
    ...
    public void process() {
        ...
        do {
            linksSlice = linkService.findLinksByMarketName(MarketName.ALIEXPRESS, pageRequest);

            List<Link> links = linksSlice.getContent();
            Map<Long, ProductDto> productDtoByLinkId = aliexpressService.fetchProducts(links);
